...
plot_cohort_panels <- function(score_file, coefficient_file, output_dir, cancer) {
  x <- read.csv(score_file, check.names = FALSE)
  required <- c("OS_time", "OS_status", "linear_predictor", "risk_group")
  if (!all(required %in% names(x))) stop("Training score file lacks required columns")
  x$risk_group <- factor(x$risk_group, levels = c("High", "Low"))
  dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
  pal <- c(High = "#D55E00", Low = "#0072B2")

  # Panel F: Kaplan-Meier curves and risk table; log-rank P value from survdiff.
  km <- survival::survfit(survival::Surv(OS_time, OS_status) ~ risk_group, data = x)
  km_plot <- survminer::ggsurvplot(km, data = x, pval = TRUE, risk.table = TRUE,
  ...
  roc <- timeROC::timeROC(T = x$OS_time, delta = x$OS_status, marker = x$linear_predictor,
  ...
  x <- x[order(x$linear_predictor), ]; x$patient_index <- seq_len(nrow(x)); x$status <- factor(ifelse(x$OS_status == 1, "Death", "Alive"), levels = c("Death", "Alive"))
  p1 <- ggplot2::ggplot(x, ggplot2::aes(patient_index, linear_predictor, colour = risk_group)) + ggplot2::geom_point(size=.6) + ggplot2::scale_colour_manual(values=pal) + ggplot2::theme_classic(base_size=7) + ggplot2::labs(x=NULL,y="Linear predictor",colour="Risk group")
  p2 <- ggplot2::ggplot(x, ggplot2::aes(patient_index, OS_time, colour = status)) + ggplot2::geom_point(size=.6) + ggplot2::scale_colour_manual(values=c(Death="#D55E00",Alive="#0072B2")) + ggplot2::theme_classic(base_size=7) + ggplot2::labs(x="Patient index",y="Overall survival (days)",colour="Status")
  genes <- read.csv(coefficient_file)$gene; expression <- as.matrix(x[, genes, drop=FALSE]); scaled <- t(scale(expression));
  annotation <- data.frame(Risk_group=x$risk_group); rownames(annotation) <- x$sample_id
  pheatmap::pheatmap(scaled, cluster_rows=FALSE, cluster_cols=FALSE, show_colnames=FALSE,
  ...
  long <- reshape2::melt(x[, c("risk_group", genes), drop=FALSE], id.vars="risk_group", variable.name="gene", value.name="expression")
